...
export function ApplicationsAdminPage() {
  ...
  return (
    <>
      ...
      <Dialog open={Boolean(permissionApp)} onClose={() => setPermissionApp(null)} maxWidth="sm" fullWidth><DialogTitle>Permissions · {permissionApp?.name}</DialogTitle><DialogContent><Typography color="text.secondary" sx={{ mb: 2 }}>Administrators always have access. Other users require an enabled group below.</Typography><Stack divider={<span />}>
        {permissions.map((permission) => <FormControlLabel key={permission.group.id} sx={{ m: 0, py: 1, borderBottom: 1, borderColor: "divider", justifyContent: "space-between" }} labelPlacement="start" label={<span>{permission.group.name}<Typography component="span" color="text.secondary" sx={{ ml: 1, fontSize: 13 }}>{permission.group.enabled ? "" : "disabled"}</Typography></span>} control={<Switch checked={permission.allowed} slotProps={{ input: { "aria-label": `${permission.group.name} access` } }} onChange={(event) => void updatePermission(permission.group.id, event.target.checked)} />} />)}
        {!permissions.length && <Typography>No groups available.</Typography>}
