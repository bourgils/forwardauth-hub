import { Alert, Box, Button, Checkbox, Chip, Dialog, DialogActions, DialogContent, DialogTitle, FormControl, FormControlLabel, ListItemText, MenuItem, OutlinedInput, Paper, Select, Snackbar, Stack, Switch, Table, TableBody, TableCell, TableContainer, TableHead, TableRow } from "@mui/material";
import { useCallback, useEffect, useState, type FormEvent } from "react";
import { apiRequest } from "../../../api/client";
import { ConfirmDialog } from "../../../components/ConfirmDialog";
import { FieldLabel, LabeledTextField } from "../../../components/FormField";
  ...
  return (
    <>
      ...
      <Dialog open={Boolean(form)} onClose={() => setForm(null)} maxWidth="sm" fullWidth><Stack component="form" onSubmit={(event) => void save(event)}><DialogTitle>{form?.id ? "Edit user" : "Add user"}</DialogTitle><DialogContent><Stack spacing={2} sx={{ pt: 1 }}>
        ...
        <FormControl><FieldLabel id="user-groups-label" htmlFor="user-groups">Groups</FieldLabel><Select id="user-groups" labelId="user-groups-label" multiple displayEmpty value={form?.groupIds ?? []} onChange={(event) => setForm((value) => value && ({ ...value, groupIds: typeof event.target.value === "string" ? event.target.value.split(",") : event.target.value }))} input={<OutlinedInput />} renderValue={(selected) => selected.length ? groups.filter((group) => selected.includes(group.id)).map((group) => group.name).join(", ") : <Box component="span" sx={{ color: "text.disabled" }}>Select groups</Box>}>
