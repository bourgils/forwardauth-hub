...
import { adminNavigationSections } from "./adminNavigation";
    ...
    <Box sx={{ minHeight: "100vh" }}>
      ...
      <Box sx={{ display: { md: "grid" }, gridTemplateColumns: "230px minmax(0, 1fr)", minHeight: "calc(100vh - 64px)" }}>
        <Box component="aside" sx={(theme) => ({ borderRight: { md: `1px solid ${theme.palette.divider}` }, borderBottom: { xs: `1px solid ${theme.palette.divider}`, md: 0 }, p: 2 })}>
          <AdminCommandPalette />
          <Stack component="nav" aria-label="Administration" direction={{ xs: "row", md: "column" }} sx={{ mt: 2, overflowX: "auto", gap: 2 }}>
            {adminNavigationSections.map((section) => <Box key={section.label} sx={{ flex: "0 0 auto" }}>
